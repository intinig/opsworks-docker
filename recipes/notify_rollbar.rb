    ...
    c.each do |app_name, app_config|
      if app_config["notifications"] && app_config["notifications"]["rollbar"]
        execute "notifying rollbar of #{app_name} deployment" do
          ...
          cmd = "docker exec #{app_name}0 sh -c 'curl https://api.rollbar.com/api/1/deploy/ -F access_token=#{access_token} -F environment=$#{env_var} -F revision=$#{rev_var} -F local_username=#{node["opsworks"]["stack"]["name"]}'"
